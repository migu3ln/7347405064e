...
class ElencoMultimediaController extends AweController {
    ...
    public function actionAdmin() {
        $model = new ElencoMultimedia('search');
        $model->unsetAttributes(); // clear any default values
        if (isset($_GET['ElencoMultimedia']))
            $model->attributes = $_GET['ElencoMultimedia'];
    ...
    public function loadModel($id, $modelClass = __CLASS__) {
        $model = ElencoMultimedia::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model, $form = null) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'elenco-multimedia-form') {
            echo CActiveForm::validate($model);
            Yii::app()->end();
        }
    }

    public function actionUploadTmps() {
        $carpeta = 'tmp';
        $id = '';
//        die(var_dump("@@ss"));
